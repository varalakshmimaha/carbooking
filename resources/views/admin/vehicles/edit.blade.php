    ...
    <div class="py-12 bg-gray-50">
        <div class="max-w-7xl mx-auto px-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <form action="{{ route('admin.vehicles.update', $vehicle) }}" method="POST" enctype="multipart/form-data" class="p-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="cab_name" required value="{{ old('cab_name') ?? $vehicle->cab_name }}" placeholder="Enter Cab name" class="w-full border-gray-200 rounded-xl focus:ring-blue-500 focus:border-blue-500">
                        </div>

                        <!-- Model Year -->
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Model Year <span class="text-red-500">*</span></label>
                            <input type="text" name="model_year" required value="{{ old('model_year') ?? $vehicle->model_year }}" placeholder="e.g. 2024" class="w-full border-gray-200 rounded-xl focus:ring-blue-500 focus:border-blue-500">
                        </div>

                        <!-- Cab Number -->
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Cab Number <span class="text-red-500">*</span></label>
                            <input type="text" name="cab_number" required value="{{ old('cab_number') ?? $vehicle->cab_number }}" placeholder="e.g. KA 01 AB 1234" class="w-full border-gray-200 rounded-xl focus:ring-blue-500 focus:border-blue-500">
                        </div>

                        <!-- Chassis Number -->
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Chassis Number <span class="text-red-500">*</span></label>
                            <input type="text" name="chassis_number" required value="{{ old('chassis_number') ?? $vehicle->chassis_number }}" placeholder="Enter Chassis Number" class="w-full border-gray-200 rounded-xl focus:ring-blue-500 focus:border-blue-500">
                        </div>

                        <!-- Cab Color -->
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Cab Color <span class="text-red-500">*</span></label>
                            <input type="text" name="cab_color" required value="{{ old('cab_color') ?? $vehicle->cab_color }}" placeholder="Enter Cab Color" class="w-full border-gray-200 rounded-xl focus:ring-blue-500 focus:border-blue-500">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="with_carrier" required class="w-full border-gray-200 rounded-xl focus:ring-blue-500 focus:border-blue-500">
                                <option value="Yes" {{ (old('with_carrier') ?? $vehicle->with_carrier) == 'Yes' ? 'selected' : '' }}>Yes</option>
                                <option value="No" {{ (old('with_carrier') ?? $vehicle->with_carrier) == 'No' ? 'selected' : '' }}>No</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="seating_capacity" required value="{{ old('seating_capacity') ?? $vehicle->seating_capacity }}" placeholder="Enter Seating Capacity" class="w-full border-gray-200 rounded-xl focus:ring-blue-500 focus:border-blue-500">
                        </div>
                    </div>
                        ...
                        @foreach($images as $key => $label)
                            <div class="space-y-4">
                                ...
                                <div class="border-2 border-dashed border-gray-200 rounded-2xl p-6 flex flex-col items-center justify-center bg-gray-50 hover:bg-gray-100 transition-all cursor-pointer relative min-h-[200px]" onclick="document.getElementById('file_{{ $key }}').click()">
                                    @if($vehicle->$key)
                                        <img id="preview_{{ $key }}" src="{{ asset('storage/' . $vehicle->$key) }}" class="absolute inset-0 w-full h-full object-cover rounded-2xl">
                                    @else
                                ...
                                <select name="{{ $key }}_verified" class="w-full border-gray-200 rounded-xl text-sm focus:ring-blue-500">
                                    <option value="Unverified" {{ (old($key.'_verified') ?? $vehicle->{$key.'_verified'}) == 'Unverified' ? 'selected' : '' }}>Unverified</option>
                                    <option value="Verified" {{ (old($key.'_verified') ?? $vehicle->{$key.'_verified'}) == 'Verified' ? 'selected' : '' }}>Verified</option>
                                    <option value="Pending" {{ (old($key.'_verified') ?? $vehicle->{$key.'_verified'}) == 'Pending' ? 'selected' : '' }}>Pending</option>
                                </select>
                            </div>
                        @endforeach
                    </div>
